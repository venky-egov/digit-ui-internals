...
export const SuccessfulPayment = (props) => {
  ...
  const { isLoading, data: payments } = Digit.Hooks.usePaymentUpdate({ egId });
  ...
  const bannerText = `CITIZEN_${
    paymentData?.paymentStatus !== "DEPOSITED" ? "FAILURE" : "SUCCESS"
  }_${paymentData?.paymentDetails[0].businessService.replace(/\./g, "_")}_PAYMENT_MESSAGE`;
  ...
  return (
    <Card>
      <Banner
        ...
        successful={paymentData?.paymentStatus !== "DEPOSITED"}
      />
      {paymentData?.paymentStatus !== "DEPOSITED" && (
        <React.Fragment>
