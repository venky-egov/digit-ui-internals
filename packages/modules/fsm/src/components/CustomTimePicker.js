...
const CustomTimePicker = ({ name, value, onChange }) => {
  ...
  if (timeFormat.includes('AM') || timeFormat.includes('PM')) {
    return (
      <TextInput name={name} type="time" value={value} onChange={event => onChange(event.target.value)} style={{ width: "100%", maxWidth: "200px" }} />
    )
  }

  return (
    <TimePicker name={name} onChange={onChange} value={value} locale="en-US" disableClock={false} clearIcon={null} maxLength={2} onKeyDown={onTimeKeyDown} />
  )
}
